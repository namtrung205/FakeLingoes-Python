...
class TranslateMainWindow(QWidget):
	...
	def __init__(self, parent = None):
		...
		self.transShortcut = QShortcut(QKeySequence("Ctrl+D"),self)
		...
		self.hideShortcut = QShortcut(QKeySequence("Ctrl+H"),self)
		self.hideShortcut.activated.connect(self.hideButton_Clicked)

		self.swapShortcut = QShortcut(QKeySequence("Ctrl+W"),self)
		self.swapShortcut.activated.connect(self.swapButton_Clicked)

		self.capImage_ApiShortcut = QShortcut(QKeySequence("Ctrl+2"),self)
		self.capImage_ApiShortcut.activated.connect(self.captureApiButton_click)

		self.capImageShortcut = QShortcut(QKeySequence("Ctrl+1"),self)
		...
		# Content Mean
		self.contentMeaning = ""
		self.meaningdWindow.meaningBoxEn.textCursor().insertHtml(self.contentMeaning)
	...
	def alwayTop_enable(self):
		if not self.show_action.isChecked():
			...
			self.move(widthMonitor - self.width(), 0)			
			self.show()
		else:
			self.setWindowFlags(
			Qt.Window |
	...
	def transButton_Click(self, text = ""):
		...
		try:
			...
			outText = myTimer(googleTrans,mytext, self.myTimeOut )
	...
	def largeButton_Click(self):
		...
		if self.height() <=150:
			...
			self.setFixedSize(250, heightMonitor)
			self.move(widthMonitor - self.width(), 0)
	...
	def showMeaning(self):
		try:
			self.meaningdWindow.meaningBoxEn.setPlainText("")
			self.contentMeaning = OxFordDic().getHtml(self.inputBox.toPlainText())
			self.meaningdWindow.meaningBoxEn.textCursor().insertHtml(self.contentMeaning)
		except NameError:
			print("Error 5")


	def moreInfoShow(self):
		try:
			self.showMeaning()
			meaningTransToVi = googleTrans(self.contentMeaning, 'en', 'vi')
			meaningTransToVi = meaningTransToVi.replace("/ ", "/")

			self.meaningdWindow.meaningBoxVi.setPlainText("")
			self.meaningdWindow.meaningBoxVi.textCursor().insertHtml(meaningTransToVi)
