...
class MeaningWindow(QWidget):
	...
	def __init__(self,Oxford_appID, Oxford_appKey,word="",parent = None):
		super(MeaningWindow, self).__init__(parent)

		self.Oxford_appID = Oxford_appID
		self.Oxford_appKey = Oxford_appKey
	...
	def refeshMeaningButton_click(self):
		...
		try: 
			...
			if self.dictComBo.currentText() =="OxFord":
				...
				# myTimer(func, args, timeout)
				myOxFordDict = OxFordDic(self.Oxford_appID, self.Oxford_appKey)
				print(self.Oxford_appID, self.Oxford_appKey)
				self.OxforHtml = myTimer(myOxFordDict.getHtml, (self.inputWord,), 8)
				self.meaningBoxEn.textCursor().insertHtml(self.OxforHtml,)
				self.meaningBoxEn.repaint()
				...
				self.SoHoaVi_Vihtml = "<h2>%s</h2>" %self.inputWord + myTimer(SoHoaDic().getFullHtmlVi, (self.inputWord,), 8)
				self.meaningBoxViSoHoa.textCursor().insertHtml(self.SoHoaVi_Vihtml)
				# self.meaningBoxViSoHoa.textCursor().insertHtml("<h2>%s</h2>" %self.inputWord + SoHoaDic().getFullHtml(self.inputWord,"en_vi"))
				self.meaningBoxViSoHoa.repaint()

				self.meaningBoxEnSoHoa.setPlainText("")
				self.SoHoaEn_EnHtml = "<h2>%s</h2>" %self.inputWord + myTimer(SoHoaDic().getFullHtmlEn, (self.inputWord,), 7)
				self.meaningBoxEnSoHoa.textCursor().insertHtml(self.SoHoaEn_EnHtml)
				self.meaningBoxEnSoHoa.repaint()
	...
	def transMeaningButton_click(self):
		...
		try:
			if self.dictComBo.currentText() == "So Hoa":
				...
				SoHoaEn_EnHtml = self.SoHoaEn_EnHtml
				print("Chieu dai html")
				print(len(SoHoaEn_EnHtml))
				if len(SoHoaEn_EnHtml) > 5000:
					SoHoaEn_EnHtml = SoHoaEn_EnHtml[0:4999]
				print(len(SoHoaEn_EnHtml))
				self.meaningBoxEn_ViSoHoa.setPlainText("")
				# transMeanEnToVi = googleTrans(SoHoaEn_EnHtml, "en", "vi") # Non limit time
				transMeanEnToVi = myTimer(googleTrans,(SoHoaEn_EnHtml, "en", "vi"), 8)
				transMeanEnToViHtml = transMeanEnToVi.replace("/ ", "/")
				self.meaningBoxEn_ViSoHoa.textCursor().insertHtml(transMeanEnToViHtml)
				self.meaningBoxEn_ViSoHoa.repaint()
				...
				transMeanToVi = myTimer(googleTrans, (self.OxforHtml, "en", "vi"), 7)
				# print(transMeanToVi)
